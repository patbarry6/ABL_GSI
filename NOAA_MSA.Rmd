...
### *Bayes2rubias*
Chuck mentioned that he would love a Bayes to rubias script. It might be easier to make a Bayes to Genepop and just run the Genepop to Bayes script from above. 
...
```
I spent a little bit of time creating code that will read in the Bayes mixture file and convert it to a genepop formatted file. Why convert it to Genepop? Well it might be useful to run it through ade4 and a individual based PCA to see how different the samples are based on their multilocus genotype. Also, genepop is the most widely used input file and most of my code starts with a Genepop file, so it is easy to pipe it into another script to convert to rubias. 

The only snafu I ran into was that the original mixture file had only a single allele called for some individuals at locus 6. It doesn't appear to be a haploid locus, but it may be that ABL scores their insertions/deletions as single alleles? I sent Chuck an email to confirm.
